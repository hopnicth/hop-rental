...
export default defineEventHandler(async (event) => {
  const { adminClient, userId: adminUserId } = await requirePlatformAdmin(event);
  ...
  if (!profileUserId && !phone) {
    throw createError({ statusCode: 400, statusMessage: "userId or phone is required" });
  }
  if (!file?.data) {
    throw createError({ statusCode: 400, statusMessage: "ID card image is required" });
  }
  if (!ALLOWED_MIME.has(file.type || "")) {
    throw createError({ statusCode: 415, statusMessage: "Only JPEG/PNG/WebP images are supported" });
  }
  const buffer = Buffer.from(file.data);
  if (buffer.byteLength > MAX_BYTES) {
    throw createError({ statusCode: 413, statusMessage: "ID card image must be 10MB or smaller" });
  }

  const ownerKey = normalizeStorageKey(profileUserId || phone);
  const path = `customer-ids/${ownerKey}/${crypto.randomUUID()}.${extFromMime(file.type || "")}`;
  ...
  const idCardUrl = adminClient.storage.from(CATALOG_MEDIA_BUCKET).getPublicUrl(path).data.publicUrl;

  if (profileUserId) {
    const update: Record<string, unknown> = { id_card_url: idCardUrl };
    if (phone) update.phone = phone;
    if (fullName) update.full_name = fullName;
    const { error } = await adminClient.from("users").update(update).eq("id", profileUserId);
  ...
  return { idCardUrl, storageBucket: CATALOG_MEDIA_BUCKET, storagePath: path };
});