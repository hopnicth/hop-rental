...
--
-- Add a public asset catalog layer with admin-managed
-- product matching, asset documents/service/checklist support,
-- and booking snapshot fields rooted at asset.
-- ============================================================
...
COMMENT ON TABLE public.assets IS 'Public/commercial rental catalog root separated from sale products and raw rental assets.';
COMMENT ON COLUMN public.assets.code IS 'Stable business code used by staff/admin and snapped into bookings.';
...
CREATE TABLE public.asset_matches (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.asset_documents (
  id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  asset_id   UUID NOT NULL REFERENCES public.assets(id) ON DELETE CASCADE,
  service_event_id   UUID REFERENCES public.asset_service_events(id) ON DELETE SET NULL,
  document_kind      public.asset_document_kind NOT NULL DEFAULT 'other',
  visibility         public.asset_document_visibility NOT NULL DEFAULT 'internal',
  title              TEXT NOT NULL,
  description        TEXT,
...
CREATE TABLE public.asset_checklist_templates (
  ...
  UNIQUE (asset_id, kind, name, version)
);
...
CREATE TABLE public.asset_checklist_template_items (
  id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  template_id    UUID NOT NULL REFERENCES public.asset_checklist_templates(id) ON DELETE CASCADE,
  sort_order     INTEGER NOT NULL DEFAULT 0,
  label          TEXT NOT NULL,
  instruction    TEXT,
...
ALTER TABLE public.rental_bookings
  ...
  ADD COLUMN asset_slug TEXT,
  ADD COLUMN asset_name TEXT,
  ADD COLUMN asset_thumbnail TEXT,
  ADD COLUMN asset_snapshot JSONB NOT NULL DEFAULT '{}'::JSONB,
  ADD COLUMN matched_product_id TEXT REFERENCES public.products(id) ON DELETE SET NULL,
  ADD COLUMN matched_product_name TEXT,
  ADD CONSTRAINT rental_bookings_asset_snapshot_object_check
...
CREATE TABLE public.rental_booking_checklists (
  ...
  template_id          UUID REFERENCES public.asset_checklist_templates(id) ON DELETE SET NULL,
  kind                 public.rental_checklist_kind NOT NULL,
  template_name        TEXT,
...
CREATE TABLE public.rental_booking_checklist_items (
  ...
  template_item_id     UUID REFERENCES public.asset_checklist_template_items(id) ON DELETE SET NULL,
  sort_order           INTEGER NOT NULL DEFAULT 0,
  label                TEXT NOT NULL,
  instruction          TEXT,
  response_type        public.rental_checklist_item_response_type NOT NULL DEFAULT 'check',
...
CREATE INDEX idx_assets_status_hidden_sort
  ON public.assets(status, is_hidden, sort_order, created_at DESC);

CREATE INDEX idx_assets_last_rented_at
  ON public.assets(last_rented_at DESC);

CREATE INDEX idx_asset_matches_product_sort
  ON public.asset_matches(product_id, sort_order, asset_id);

CREATE INDEX idx_asset_matches_asset_sort
  ON public.asset_matches(asset_id, sort_order, product_id);

CREATE INDEX idx_asset_service_events_asset_date
  ON public.asset_service_events(asset_id, service_date DESC);

CREATE INDEX idx_asset_documents_asset_visibility_sort
  ON public.asset_documents(asset_id, visibility, sort_order);

CREATE INDEX idx_asset_documents_service_event
  ON public.asset_documents(service_event_id);

CREATE INDEX idx_asset_checklist_templates_asset_kind
  ON public.asset_checklist_templates(asset_id, kind, is_active, sort_order);

CREATE INDEX idx_asset_checklist_template_items_template_sort
  ON public.asset_checklist_template_items(template_id, sort_order);

CREATE INDEX idx_rental_bookings_asset_status_period
  ON public.rental_bookings(asset_id, status, start_date, end_date);
...
CREATE TRIGGER set_asset_matches_updated_at
  BEFORE UPDATE ON public.asset_matches
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

CREATE TRIGGER set_asset_service_events_updated_at
  BEFORE UPDATE ON public.asset_service_events
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER set_asset_checklist_templates_updated_at
  BEFORE UPDATE ON public.asset_checklist_templates
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

CREATE TRIGGER set_asset_checklist_template_items_updated_at
  BEFORE UPDATE ON public.asset_checklist_template_items
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE POLICY "asset_matches_select_public"
  ON public.asset_matches FOR SELECT
  USING (
    EXISTS (
      SELECT 1
      FROM public.assets ra
      WHERE ra.id = asset_matches.asset_id
    ...
    AND EXISTS (
      ...
      WHERE p.id = asset_matches.product_id
        AND p.is_hidden = FALSE
    )
  );

CREATE POLICY "asset_documents_select_public_or_customer"
  ...
  USING (
    (
      ...
      AND EXISTS (
        SELECT 1
        FROM public.assets ra
        WHERE ra.id = asset_documents.asset_id
          AND ra.status = 'active'
          AND ra.is_hidden = FALSE
      )
    )
    OR (
      ...
      AND EXISTS (
        ...
        WHERE rb.asset_id = asset_documents.asset_id
