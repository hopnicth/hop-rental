...
- Post-demotion sweep of all elevated roles on the project: 1 `super_admin` (`c1089c0e`, the genuine owner account) + 4 `staff`. Among the staff is `smoke-staff@hopnic-test.local` (`5418f2df`) — a pre-existing test account at `staff`, first recorded in `docs/audit/2026-07-15-remote-cleanup-and-migration-sync.md`. It is **out of scope of this ruling and was not touched**; flagged here so a future disposition decision is not lost.
