...
- [PARTIAL 2026-07-26][post-merge] `app/types/database.types.ts` regen — **REGEN DONE** in 3bc7592 (`supabase gen types typescript --linked`, remote catalog at mig-146, purely additive 226 lines; run WITHOUT `--schema public`, which silently drops the graphql_public block). REMAINING: remove the now-unnecessary boundary casts at settlement-waive.post.ts and the launch-cancel endpoints (return-settlement.get.ts's own cast was already removed). Do NOT hand-edit the generated file. OWNER: post-merge batch 1.
...
- [T-LAUNCH][BEFORE BATCH 1] Remove the staff-charge channel + add the settlement memo field (decisions.md 2026-07-26 a/b/c/d). The web collects ONLY rental_charge + rental_extension. REMOVE from the application layer: staff free-form typed charge lines, the 5,000 THB per-line cap, and the taxable_service_charge / actual_damage / contractual_penalty paths — `server/utils/rental-return-settlement.ts`, `server/api/admin/rental-bookings/[id]/return-settlement.post.ts`, and the admin settlement UI. ADD: a TEXT-ONLY memo field on the return/settlement flow for overdue days (late_days is already computed by mig-134) and adjustment notes — no amount, no charge line, no document, no effect on the settlement total. Discount tiers (staff 20% / super_admin 50%) stay, on the rental base. `pending_review` STAYS as a dormant fail-closed backstop (auditor ruling 2026-07-26) — it is a guard, not a collection path. REMOVAL SCOPE per decisions 2026-07-26 d: application paths removed; DB vocabulary narrowed for NEW WRITES ONLY; historical append-only rows citing the removed types remain valid; no history rewrite, no destructive migration. OPEN QUESTION for this task's own gate: whether `f_settle_rental_booking_return` keeps its `p_staff_charge_lines` argument (accepting only empty) or changes signature again — mig-143 already dropped the 10-arg form to create the current 13-arg one, and that signature was the R-A merge-blocker. DO NOT sweep in `server/utils/rental-booking-no-show.ts:282-283` — its `contractual_penalty_damage_deposit_forfeiture` / `non_vat_contractual_penalty` strings are `revenue_category`/`tax_treatment` values on the parked DEPOSIT path, a different vocabulary. TESTS: `tests/server/rental-return-settlement.spec.ts` asserts the removed behaviour (5,000 cap refusal, disabled-type refusal, a taxable_service_charge success) — updating those assertions is part of this task.
- [MED][UI][GLOSSARY] TWO MORE customer-facing RAW deposit-status renders, beyond the rental-detail badge: `app/pages/user/documents/[id]/print.vue:496` (a customer DOCUMENT print) and `app/pages/rental-booking-payment/[bookingId].vue:177,242`. Both interpolate the raw enum, and both are customer money surfaces, so both fall under the standing deposit DATA vs deposit DISPLAY constraint (decisions.md 2026-07-26 addendum) — the print page doubly so, since that ruling binds every document-layer template. FIX SHAPE = the badge's: hide when no deposit exists, gated on a server-authoritative signal, deposit-era rendering preserved for revival; plus real labels for the enum. Surfaced by the task-1 scoping audit (finding A9) and auditor-ruled OUT of the badge task on 2026-07-26 — this line is that ruling's record, written late (it belonged in the 2a/2b docs).
- [PARTIAL 2026-07-27][UI][GLOSSARY] Customer rental detail header deposit chip — **BADGE DONE** (feature/deposit-badge): the chip is gated on a server-authoritative `depositsEnabled` (`f_deposits_enabled`, fail-closed) on the customer detail payload, NOT on `isLaunchBooking`; at launch the slot renders EMPTY with no replacement text (CHiP ruling a-ก), and the deposit-era markup is preserved behind the flag. **STILL DEFERRED: the raw-enum leak** — the chip interpolates the RAW status into Thai copy ("สถานะมัดจำ: unpaid"), so deposit-era bookings would still render an untranslated English state. Fixing it needs NEW i18n keys and Thai that exists in neither locale. DEFERRED PENDING DISPOSITION of the remote inventory (2026-07-27): remote holds exactly ONE deposit-era booking (`02004e7d…d111`, cancelled, deposit refunded) and it is SMOKE TEST RESIDUE — so if CHiP rules that row out of scope, no real customer can ever reach the enum labels and the work is moot. Re-open only for a real deposit-era booking or at deposit revival.
- [MED][UI][GLOSSARY] Customer rental detail DOCUMENTS card still carries deposit vocabulary on a launch booking: `rentalsPage.documents.unavailable.bookingConfirmation` renders "…ชำระ Booking Deposit สำเร็จ" (th) / "Available after the booking is confirmed and Booking Deposit is paid." (en). NOT a v-if fix, which is why it is deferred rather than folded into the badge task: the hint is driven by `canIssueBookingConfirmation`, which REQUIRES `bookingDepositPaymentStatus === "paid"`, so a launch booking can never issue its own booking-confirmation document at all. The copy is a symptom; the real question is what issues a launch booking's confirmation and when — T4 document-layer scope. Surfaced by the 2026-07-27 deposit-badge walk after the chip and the three money-summary deposit cells were gated. Same DATA-vs-DISPLAY principle (decisions.md 2026-07-26 addendum).
...
- [DISCLOSE 2026-07-27][ACCOUNTING] BDR-202607-0001 and -0002 were consumed by the 2026-07-21 remote smoke walk (booking 02004e7d…d111, since cancelled and refunded), so the first REAL deposit-refund document of 2026-07 cannot be -0001. Numbers are sequence-allocated and the void/reissue chain is immutable — nothing to remediate. CHiP 2026-07-27: DISCLOSE only; the period-prefixed format (BDR-YYYYMM-NNNN) contains the impact to that period. Carry to the accountant meeting for awareness alongside the other DISCLOSE items. Ref: docs/audit/2026-07-27-remote-legacy-booking-inventory.md.
- [DISCLOSE — unchanged] VAT treatment = inclusive; invoice dating = staff issue date (diverges from ม.78/1). Decided, not open; carry to the meeting. Ref design doc §8.10/§8.11.
