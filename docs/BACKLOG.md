...
- [POLICY][CHiP RULING PENDING] Unreachable-customer waiting-period policy: days threshold that must elapse (refund row failed / needs_customer_contact exhausted) before booking_deposit_forfeiture_refund_unreachable_v1 may be used to close the ledger (design doc §A deferred-release guard, exit b).
...
- [LAUNCH][ACCOUNTING][N2] Return-payment tax point + the awaiting_payment guard: confirm service tax point = receipt of payment (ม.78/1), invoice dated the payment date, engine refuses any tax doc while settlement awaiting_payment (decisions.md d). Ref report 3 N2.
