...
- [MED][UI][GLOSSARY] Customer rental detail header shows "สถานะมัดจำ: unpaid" on launch bookings — implies an obligation that does not exist and surfaces bare มัดจำ. FIX = hide the badge when the booking carries no deposit (gate on deposit state / feature-gate) — data untouched, deposit-era rendering preserved for revival. Scheduled: next session, before or alongside post-merge batch 1. (CHiP ruling 2026-07-25/26: remove from display, keep as data. Standing principle: decisions.md 2026-07-26 addendum, deposit DATA vs deposit DISPLAY.) Site: `app/pages/user/rentals/[bookingId].vue:409-413`, key `rentalsPage.detail.depositStatus`. IMPLEMENTATION NOTE: do NOT gate on the existing `isLaunchBooking` predicate (`:68`, `bookingDepositPaymentStatus !== "paid"`) — it also matches deposit-era `pending`/`failed`/`expired` bookings, whose deposit status the customer SHOULD still see. Gate on the deposits feature flag or on "no deposit exists" instead. SECOND DEFECT ON THE SAME LINE: the badge interpolates the RAW ENUM into Thai copy ("สถานะมัดจำ: unpaid"), so even on deposit-era bookings it renders an untranslated English state — worth fixing in the same pass.
