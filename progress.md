...
- **Migration 127 GATED (conditional pass → proof delivered → approved byte-as-submitted), applied via `supabase db reset --local` (full 001→127 chain clean), post-apply constraint state verified, suite 130 files / 2698 tests green.** Widened: disposition source vocabulary + XOR (late_cancellation_forfeiture), action-log vocabulary (incl. return_refund_resume paying the T2 BACKLOG debt), payment_refunds.refund_method.
- **GATE ORDERING (for closing-package reconciliation): gates ride their walks, not their numbers — 129 gates BEFORE 128 (129's rental cancel RPCs + 7-day tier unblock walks 1-3; 128 must gate before walk 4 starts). Acknowledged at gate 127.**
- Migration 129 GATED (held → content approved as amended → passed with 127 ratification), applied via full-chain reset, verification pasted (SECURITY DEFINER, service_role-only EXECUTE), suite 2698/2698 green.
- Walk 1 leg 1 DONE: customer tier −3→−7 + policy `booking_deposit_refund_calendar_day_v2` + fixture shifts (3 specs, 42/42 targeted, tsc clean, full suite green).
- **Migration RENUMBERING (CHiP-ratified at gate 130): 130 = mark-refunded ledger-release RPC · 131 = void-reissue · 132 = money-ops decision log · 128 unchanged (sale; gates before walk 4).**
- **Standing lock convention (CHiP, gate 130): every money writer touching both rental_bookings and a money row locks the BOOKING FIRST (125/126/129 pattern); 130 amended to comply.**
- OPERATING-MODEL.md §3: gate-verdict confirmation rule added (2026-07-19, post self-adjudicated-gate ruling). NOT staged yet.
- Migration 130 GATED (conditional pass → 2 amendments → CONFIRMED open), applied (001→130 reset clean), verification pasted, suite green. Lock convention → design annex #7.
- Wrapper leg DONE: transitionAdminRefund mark-refunded → RPC (proof now MANDATORY, ratified); endpoint passes platformRole; mock-client RPC dispatch; 1 behavior-flip test rewritten; server-utils index row updated. tsc clean, suite 2698/2698.
- WALK 1 ACCEPTED (2026-07-19). Dispositions: HTTP-level full walk = logged DEBT (BACKLOG, pre-Phase-1-close gate); walk-1 fixtures KEEP; gate 132 pulled before walk 2.
- Migration 132 GATED → CONFIRMED → applied (reset tolerates 128/131 gaps — lexical order; remote apply will need --include-all when 128/131 backfill). Verification: triggers/policies live, INSERT ok, UPDATE blocked (one permanent walk2_probe denied row from the append-only probe — immutable by design). Suite green.
- WALK 2 DONE (live TS walk via temp spec against local DB, spec deleted after): logMoneyOpsDecision util + lateCancelForfeitRentalBooking + late-cancel-forfeit endpoint (§F inversion). Evidence: 4 decision-log rows (3 denied: window_not_reached/booking_not_cancellable[walk-1 fixture]/actor_role_invalid; 1 allowed pre-RPC, fail-closed proven in spec); full 129 chain on booking 7777: cancellation event (staff/admin_rental_detail/v2), disposition late_cancellation_forfeiture ฿800 policy late_cancel_v1, recognition non_vat_contractual_penalty 800 (vat/wht 0), ledger forfeiture 800, residue 0.00, action-log late_cancel_forfeit; outside-window booking untouched. Walk-1 fixture re-seeded to terminal state post-reset (reset was required for 132). New spec 8/8; suite 131 files / 2706; tsc clean. Index rows added (admin-rental-booking-cancel, money-ops-log). Awaiting checkpoint audit.
- WALK 2 ACCEPTED (2026-07-19).
- WALK 3 DONE (live TS walk, temp spec deleted): companyCancelRentalBooking + company-cancel endpoint (§F inversion — staff denial not_super_admin LOGGED before 403, first live money-inversion denial). Evidence: decision log = 1 denied (staff) + 3 allowed (700/400/NULL); main booking full loop — company cancel → refund pending 700 → proof → mark-refunded RPC → ledger refund 700, residue 0.00; B-M1 preview: paid_confirm_failed admitted (refund pending 400); draft edge: flip+event only, no refund row, allowed amount NULL; company_cancel_refund action-log rows ×3. Spec 7/7; suite 132 files / 2713; tsc clean. Awaiting checkpoint audit.
- WALK 3 ACCEPTED (2026-07-19). BACKLOG: payment_source_unresolved reopening trigger recorded.
- Migration 128 GATED (held: closure FOR UPDATE OF bug fixed + payment_status enum proof 009:18-26 → CONFIRMED), applied (chain 127-130+132 complete; only 131 outstanding), verification pasted (3 RPCs secdef, service_role-only, guard triggers), suite green.
- WALK 4 DONE (live, temp specs deleted): cancelSaleOrder/settleSaleOrderRefund wrappers + cancel/settle endpoints; raw-flip RETIRED (patch.ts status='cancelled' → 409 SALE_ORDER_CANCEL_MOVED, source-contract pinned). D5/BUGS 1-3 closed with evidence: stock 8→10 restored pre-fulfillment (BUG 1), statuses truthful — cancelled/refunded/settled vs paid-until-settle vs cancelled-unpaid (BUG 2), refund rows exist w/ derived amounts 350+175 (BUG 3); single-target request auto-cancelled, multi-target left open+counted; post-fulfillment refuses restore (manual flagged, stock unchanged); settle-without-evidence 422; double-cancel idempotent (wrapper replay fix found by the live walk — replay short-circuits with no decision row). Decision log: staff not_super_admin denial + allowed rows + rpc_failed_after_allow corridor. Note: one denied/order_not_cancellable row on 0001 from the pre-fix e2 attempt (immutable, honest). Spec 9/9; suite 133 files / 2722; tsc clean. Awaiting checkpoint audit.
- WALK 4 ACCEPTED (2026-07-19). RULING executed: paymentStatus raw path CLOSED — 'refunded'/'cancelled' subtracted from ORDER_PAYMENT_STATUS_TRANSITIONS (table now paid:[]), patch.ts 409 SALE_ORDER_PAYMENT_STATUS_MOVED before validation, both source-pinned (admin UI dropdown inherits the subtraction automatically).
- WALK 5 DONE (live, temp spec deleted): cancelMixedPostPaymentPair orchestrator (rental leg FIRST → sale leg → session flip; no cross-domain txn; finalized allocations NOT voided — money history; resume = verified-and-skipped with reason-mismatch 409). Live: crash simulation (rental leg alone) → visible half state (booking cancelled / order confirmed) → mismatched resume 409 (sale untouched) → matching resume completed sale leg + session cancelled → full replay idempotent. B4 closed: pair fully cancelled, rental refund pending_admin_review ฿200 + sale refund pending ฿175, stock 10→11 restored, decision log = company_cancel allowed 200 + sale_cancel_paid allowed 175 (one op per leg). Spec 5/5 + closure pins 11/11; suite 134 files / 2729; tsc clean. Awaiting checkpoint audit.
- WALK 5 ACCEPTED (2026-07-19).
- WALK 6 DONE (live, temp specs deleted): B-M1 surface (stuck-deposits list + retry-confirm endpoint, both requirePlatformAdmin; refund exit routes to company-cancel). Live: surface listed both stuck fixtures w/ money context → exit 1 retry-confirm recovered A (confirmed, deposit back to paid, failure fields cleared, ledger still exactly 1 collection — no double) after two FIXTURE gaps surfaced by validation (hub_id required; asset must be active w/ matching pricing — realistic-checkout invariants, not code bugs) → replay idempotent → exit 2 refunded B via company-cancel (cancelled, refund pending 250) → surface EMPTY. TWO REAL FINDINGS fixed+pinned: (1) surface predicate must exclude cancelled bookings (company cancel keeps paid_confirm_failed as payment history — B would have been listed forever); (2) companyCancelRentalBooking gained the walk-4 idempotent-replay short-circuit (was logging denied/booking_not_cancellable on replay; late-cancel deliberately stays strict-409 per walk-2 accepted semantics). Honest residue: one denied/booking_not_cancellable row on B from the pre-fix replay. Specs 6→8 + company-cancel 8/8; suite 135 files / 2737; tsc clean. Awaiting checkpoint audit.
- WALK 6 ACCEPTED (2026-07-19). Gate 131 CONFIRMED (v_prev_status amendment folded; self-flag credited).
- Migration 131 applied (chain 127-132 CONTIGUOUS — all Phase-1 migrations in). Verification: both RPCs secdef + service_role-only, DELETE+TRUNCATE guards live.
- WALK 7 DONE (live, temp spec deleted): void wrapper + reissue helper + void endpoint (§F document_void). Live chain on a REAL document minted through the REAL issuance path (company cancel → mark-refunded → BDR-202607-0001): staff denial logged → void (voided + 'voided' event w/ voidedFromStatus=issued) → double-void idempotent → reissue BDR-202607-0002 (fresh number, sequence last_number=2, snapshot cloned, original_document_id linked) → flip → original 'replaced' + 'replaced' event carrying replacement no. Chain walk backward pasted. Negatives: draft void DOCUMENT_NOT_VOIDABLE, wrong-linkage flip DOCUMENT_NOT_VOIDED, DELETE blocked by 131 guard, totals UPDATE blocked by 068 guard post-journey. Decision log: denied not_super_admin + allowed 600. Spec 7/7; suite 136 files / 2744; tsc clean. Residue: one draft probe doc (undeletable by design — self-labeling test_draft_doc). Awaiting checkpoint audit.
- WALK 7 ACCEPTED (2026-07-19).
- WALK 8 DONE (final walk — live): (a) cancel-first: A late-cancel forfeit 111 → job skipped it (status gate); (b) cron-first: B no_show 222 → late-cancel attempt 409 BOOKING_NOT_CANCELLABLE + denied row — real cron path; (c) RACE: fresh fixture E — cancel txn held FOR UPDATE, job launched mid-txn BLOCKED 5s until COMMIT then skipped (marked:0) → E cancelled, 1 disposition (lock-wait evidence; unique-index backstop never needed). Sequencing note: first leg-c attempt consumed fixture C via the leg-a/b job run (C legitimately no_show 333 — invariant held, demo re-run on E); (d) today-start D: boundary_passed=f after 2 job runs + customer cancel CUTOFF_PASSED under v2. ONE-DISPOSITION CHECK: 0 bookings with >1 disposition. Second job run idempotent. Suite 136 files / 2744; tsc clean. 8-WALK PLAN COMPLETE. Awaiting checkpoint audit + closing-package instructions.
- WALK 8 ACCEPTED — 8-WALK PLAN CLOSED (2026-07-19).
- HTTP-DEBT WALK DONE (closing step 1): against the RUNNING dev server (user-started, --dotenv .env.local → LOCAL 54321 verified; not restarted per standing rule), sessions minted via GoTrue admin magic-link (raw-seeded auth rows needed the empty-string token fix), ALL SIX LEGS GREEN: staff company-cancel 403 + denied row over HTTP (transport-level inversion) · super company-cancel 200 refund pending 700 · proof attach via real multipart endpoint 200 · mark-refunded 200 with LIVE document BDR-202607-0003 in response+DB and ledger residue 0 · void+reissue 200 → chain 0003(replaced)→0004(issued), events issued→voided→replaced · PATCH paid→refunded 409 SALE_ORDER_PAYMENT_STATUS_MOVED. Minted sessions revoked (auth.sessions+refresh_tokens deleted), cookie files removed. Suite 136/2744 green, tsc clean. Note: a garbage cookie probes as 500 (cookie-parse behavior, pre-existing). Awaiting step-1 acceptance before the closing package.
- **WALK 1 END-TO-END (psql, local): seed confirmed booking + ฿500 ledger collection → customer self-cancel RPC (v2 7-day params) → booking cancelled+cancelled_at (B7 customer side closed), refund pending_admin_review, method manual_transfer → negative proof-less RPC = REFUND_PROOF_REQUIRED → proof attached (real-surface shape) → mark-refunded RPC: refund event 500, residue 0.00, cancel_refund action log, refund row refunded+ref+proof → replay idempotent (alreadyRefunded=true, still 1 refund event). Awaiting checkpoint audit.**
