...
| Manage homepage CMS / featured home rails      | No       | No       | No        | No                        | Yes         |
...
- `/admin` uses `platform_role`; most current MVP pages allow `staff` + `super_admin`.
- `/admin/home-content` and `/api/admin/home-content/*` are intentionally narrower and require `super_admin`.
